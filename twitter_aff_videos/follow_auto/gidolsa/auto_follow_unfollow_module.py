...
def json_save(ids: dict[str], json_name: str) -> json:
    ...
    """
    with open(f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{json_name}.json', 'w+', encoding='utf-8') as f:
...
def following_json_save(client, my_id, name):
    ...
    json_save(ids=follow_dict, json_name=f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{name}_following_id')
    return follow_dict

def new_follow_json_save(client, name):
    """新しくフォローする人をJSON保存"""
    new_follow_dict = new_follow_id(client)
    json_save(new_follow_dict, json_name=f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{name}_new_follow_id')
...
def new_follow_id_only(name) -> list:
    ...
    following: json = ujson.load(open(f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{name}_following_id.json'))
    following_list = following['id']
    print(f"JSONに保存されているフォローした人は{len(following_list)}人います")

    ### new_follow_id フォローする人のJSONを読み込む
    new_follow = ujson.load(open(f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{name}_new_follow_id.json'))
...
def unfollow_id_only(name) -> list:
    ...
    """
    ### following フォローしてる人のJSONを読み込む
    following = ujson.load(open(f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{name}_following_id.json'))
    following_list = following['id']
    print(f"JSONに保存されているフォローした人は{len(following_list)}人います")

    ### follower フォロワーのJSONを読み込む
    follower = ujson.load(open(f'/Users/ore/Documents/GitHub/DMM/twitter_aff_videos/follow_auto/gidolsa/{name}_follower_id.json'))
